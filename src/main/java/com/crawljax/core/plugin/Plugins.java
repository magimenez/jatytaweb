...
public class Plugins {
	...
	static final ImmutableSet<Class<? extends Plugin>> KNOWN_PLUGINS = ImmutableSet
	        .of(DomChangeNotifierPlugin.class, OnBrowserCreatedPlugin.class,
	                ...
	                PreCrawlingPlugin.class, OnFireEventSuccessPlugin.class);
	...
	public boolean runDomChangeNotifierPlugins(final CrawlerContext context,
		...
		if (plugins.get(DomChangeNotifierPlugin.class).isEmpty()) {
			...
			//mgimenez: new param, the browser.
			return defaultDomComparison(stateBefore, stateAfter, context.getBrowser());
		} else {
	...
	/**
	 * Load and run the OnFireEventSuccessPlugins, this call has been made 
	 * from the fireEvent when the event is fireable. the Path is the Path 
	 * leading TO this eventable (not included).
	 * @since 12/07/2016
	 * @param source
	 *            the {@link StateVertex} source of eventable able to fire.
	 * @param target
	 *            the {@link StateVertex} target of eventable able to fire.            
	 * @param path
	 *            the path TO this eventable.
	 */
	public void runOnFireEventSuccessPlugins(CrawlerContext context,
	        StateVertex source, StateVertex target, List<Eventable> path) {
		//mgimenez 12/07/2016 nuevo plugin.
		LOGGER.debug("Running OnFireEventSuccessPlugins...");
		counters.get(OnFireEventSuccessPlugin.class).inc();
		for (Plugin plugin : plugins.get(OnFireEventSuccessPlugin.class)) {
			if (plugin instanceof OnFireEventSuccessPlugin) {
				LOGGER.debug("Calling plugin {}", plugin);
				try {
					((OnFireEventSuccessPlugin) plugin).onFireEventSuccess(
					        context, source,target, path);
				} catch (RuntimeException e) {
					reportFailingPlugin(plugin, e);
				}
			}
	...
	private boolean defaultDomComparison(final StateVertex stateBefore,
	        final StateVertex stateAfter, EmbeddedBrowser browser) {
		//mgimenez: jatyta comparison behavior
		
		AutomationFormInputValuesPlugin autoPlugin = null;
		for (Plugin plugin : plugins.get(OnFireEventSuccessPlugin.class)) {
			if (plugin instanceof AutomationFormInputValuesPlugin) {
				autoPlugin = (AutomationFormInputValuesPlugin) plugin;
				break;
			}
		}
