...
public interface ExtractorManager {
	...
	/**
	 * Mark a given element menu item as checked to prevent duplicate work. 
	 * A elements is only added when it is not already in the set of 
	 * checked elements.
	 * 
	 * @param candidateElement
	 *            the element that is checked
	 * @return true if !contains(candidateElement.uniqueString)
	 */
	boolean markMenuItemChecked(CandidateElement candidateElement);
