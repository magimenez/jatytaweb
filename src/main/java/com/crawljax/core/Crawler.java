...
public class Crawler {
	...
	private void handleInputElements(Eventable eventable) {
		...
		//mgimenez: 12/09/2016 se descartan los inputs no asociados al form del eventable.
		/*
		...
		//mgimenez 29/06/16, asociaba un input a un estado posterior, 
		//cuando vuelve a recorrer un estado, se utiliza el source del Eventable
		if(eventable.getSourceStateVertex()!=null){
			formHandler.handleFormElements(formInputs, eventable.getSourceStateVertex());	
		}else{
			//cuando es la primera vez que pasa por el estado.
			formHandler.handleFormElements(formInputs, stateMachine.getCurrentState());
		}
