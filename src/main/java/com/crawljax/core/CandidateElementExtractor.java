...
public class CandidateElementExtractor {
	...
	private String menuUnderXpath;
	private String menuItemTagName;
	...
	public CandidateElementExtractor(ExtractorManager checker, @Assisted EmbeddedBrowser browser,
		...
		this.plugin = formHandler.getPlugin();
		this.menuItemTagName = plugin.menuItemTag;
		if(menuItemTagName!=null){
			menuItemTagName = menuItemTagName.toUpperCase();
		}
		this.menuUnderXpath = plugin.menuUnderXpath;
	...
	private void evaluateElements(Document dom, CrawlElement crawl,
		...
		try {
			...
			for (Element sourceElement : nodeListForCrawlElement) {
				evaluateElement(results, relatedFrame, crawl, sourceElement, dom);
			}
		...
		for (int k = 0; k < nodeList.getLength(); k++) {
			...
			boolean matchesXpath =
			        elementMatchesXpath(eventableConditionChecker, eventableCondition,
			                expressions, element);
			LOG.trace("Element {} matches Xpath={}", DomUtils.getElementString(element),
			        matchesXpath);
			/*
			...
			if (matchesXpath && !checkedElements.isChecked(id)
				...
				LOG.debug("Element {} was added", element);
				addElement(element, result, crawlElement);
			} else {
				LOG.trace("Element {} was not added", element);
			}
		}
		return result.build();
	}
	...
	private void evaluateElement(List<CandidateElement> results, String relatedFrame,
	        CrawlElement crawl, Element sourceElement, Document dom) {
		
		EventableCondition menuCondition = new EventableCondition("12345");

		menuCondition.setInXPath(menuUnderXpath);
		
		List<String> xpathList = getFullXpathForGivenXpath(dom, menuCondition);
		...
		if (eventableCondition != null && eventableCondition.getLinkedInputFields() != null
			...
			//mgimenez: set the state title name
			...
			//mgimenez: 14/11/2016, debe verificar si el elemento es visible.
			if(browser.isVisible(candidateElement.getIdentification())){
		...
		for (CandidateElement candidateElement : candidateElements) {
			...
			if( menuItemTagName.equals(candidateElement.getElement().getTagName())
					&& checkedElements.getEventableConditionChecker().checkXPathUnderXPaths(
			                XPathHelper.getXPathExpression(candidateElement.getElement()), xpathList) ){
				//corresponde a un menu item
				if(!clickOnce || checkedElements.markMenuItemChecked(candidateElement)){
					LOG.debug("Found new menu item candidate element: {} with eventableCondition {}",
					        candidateElement.getGeneralString(), eventableCondition);
				...
				if (!clickOnce || checkedElements.markChecked(candidateElement)) {
					...
					LOG.debug("Excluded candidate element: {} with eventableCondition {}",
					        candidateElement.getUniqueString(), eventableCondition);
					LOG.debug("Excluded candidate element: {} with markChecked {}",
					        candidateElement.getUniqueString(), checkedElements.markChecked(candidateElement) );
				}
			}
		}
	}
