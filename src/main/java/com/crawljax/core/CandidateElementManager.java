...
public class CandidateElementManager implements ExtractorManager {
	 ...
	 * menuCheckedElements.
	 */
	private final Collection<String> menuElements = new ConcurrentLinkedQueue<String>();
	...
	public boolean isChecked(String element) {
		...
		return elements.contains(element) || menuElements.contains(element);
	}
	...
	/* (non-Javadoc)
	 * @see com.crawljax.core.ExtractorManager#markMenuItemChecked(com.crawljax.core.CandidateElement)
	 */
	@Override
	public boolean markMenuItemChecked(CandidateElement candidateElement) {
		//no tiene en cuenta el stateName
		String generalString = candidateElement.getGeneralString();
		synchronized (elementsLock) {
			if (menuElements.contains(generalString)) {
				return false;
			} else {
				menuElements.add(generalString);
				return true;
			}
