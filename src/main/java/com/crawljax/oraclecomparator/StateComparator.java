	...
	public String getStrippedDom(EmbeddedBrowser browser) {
		...
		for (OracleComparator oraclePreCondition : oracleComparator) {
			...
			if (allPreConditionsSucceed(oraclePreCondition, browser)) {
				...
				boolean equivalent;
				//mgimenez: for jatyta testing.
				if(oracle instanceof JatytaStateComparator){
					JatytaStateComparator jatyta = (JatytaStateComparator) oracle;
					equivalent = jatyta.isEquivalent(browser);
				}else{
					// TODO dodgy code. Is the equivalence check necessary?
					 equivalent = oracle.isEquivalent("", newDom);	
				}
